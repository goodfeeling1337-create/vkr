...
- `app/api` — маршруты, сессии, разделение ролей: `/student`, `/teacher` (проверка и эталоны), `/admin` (обзор системы)
- `app/services` — сценарии (эталон, шаблон, проверка, отзыв преподавателя, упрощённый HTML-просмотр Excel)
...
Поток данных при сдаче: **Excel студента** → загрузка в `uploads/` → `engine` (парсинг секций + чекеры) → `CheckRun` / JSON-отчёт → страница попытки (дерево заданий, сравнение с эталоном). Эталон компилируется при загрузке в `reference_work_versions`.

**Задание 11 (2НФ):** автопроверка сравнивает схемы по множеству пар *(множество атрибутов, множество ключевых атрибутов)* без учёта имён таблиц; задание 13 по-прежнему учитывает имена отношений.
...
### Преподаватель: загрузка эталона

1. Вход под ролью **teacher**.
2. Раздел **«Эталоны и загрузка»** на `/teacher`: указать название теста, при необходимости **метку курса**, выбрать режим **тренировка** или **тестирование**, загрузить `.xlsx`, при необходимости включить «Опубликовать».
3. Управление версиями и настройками сдачи — в карточке работы `/teacher/reference/{id}`.

### Преподаватель: проверка работ студента

1. На `/teacher` в разделе **«Проверка работ»** отфильтровать попытки по **курсу** и по **тесту** (работе) при большом потоке.
2. Открыть попытку: в шапке отображается **студент** (имя или логин), доступен скачивание файла и **встроенный просмотр** первого листа книги.
3. Отчёт автопроверки показан **деревом заданий** (раскрывающиеся блоки). Для задания 4 строки сравнения с расхождением подсвечиваются.
4. Оставить комментарий и/или загрузить проверенный файл — студент увидит это на своей странице попытки.

### Студент

1. Вход под ролью **student**, выбор опубликованной работы, скачивание шаблона.
2. Заполнение секций «Задание №1» … «Задание №13», отправка файла (при сбое метаданных — выбор версии эталона в форме).
3. Просмотр баллов и деталей (в режиме тестирования часть подсказок может быть скрыта политикой отображения).

### Администратор

1. Вход **admin** → перенаправление на `/admin`: список пользователей, все эталоны и последние попытки по системе.
2. Для действий как у преподавателя (все работы и попытки) — ссылка **«Преподаватель»** в шапке, интерфейс `/teacher` с полным доступом ко всем работам.

### Общий технический поток

1. Система парсит секции устойчиво (поиск «Задание №N» по ячейкам), запускает проверки по заданиям, сохраняет отчёт в `check_runs`.
2. Преподаватель или студент открывают `/teacher/attempt/{id}` или `/student/attempt/{id}` и читают сохранённый JSON-отчёт через шаблоны.
