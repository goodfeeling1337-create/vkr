...
- Режим **training/testing** задаётся при загрузке эталона (внутри БД — строки `variants` со slug `mode-training` / `mode-testing` и FK `reference_work.variant_id`). Флаг «опциональные чистые связующие отношения» берётся с привязанного `Variant`; отображение деталей в testing можно доработать в шаблонах (полный отчёт хранится в БД).
